...
- Hosted data outside the separately approved Sequence 4 push and later matrix. The
...
|---|---|---|
| **RESOLVED LOCALLY IN SEQUENCE 4; HOSTED/PILOT GATES OPEN** | The live-ledger setup selects and immutably snapshots a variable kickball regulation count. The legacy aggregate form remains unchanged and cannot govern a ledger game. | `INV-10` |
| **RESOLVED LOCALLY (2026-07-29); HOSTED/PILOT GATES OPEN** | The completed-pass control accepts signed yardage and emits equal `passYards`/`recYards` deltas, and the carry control now carries a signed standalone rushing-yardage input emitting `carries +1` with a signed `rushYards` delta in one event (no schema change — the ledger runtime already allowlisted signed `rushYards`). The `INV-03` admin surface is complete locally; positive hosted proof remains with the durable pilot. | `INV-03` |
| **RESOLVED LOCALLY IN SEQUENCE 4; HOSTED/PILOT GATES OPEN** | Session start copies both teams' eligible participants; corrections clone the prior immutable participant snapshot instead of rereading the roster. | `INV-11`, `INV-12` |
...
| **RESOLVED LOCALLY IN SEQUENCE 5A; OWNER/COMMIT/HOSTED/PILOT GATES OPEN** | Migration 28 and the admin scorekeeper reuse the existing ledger for overtime, record an explicit append-only `period_close`, reject scoring after a close, return `overtime_period_open` or `continue_overtime` without mutation, and finalize only an unequal score after the latest admin-closed period. Kickball and flag-football repeated-round behavior, correction-chain handling, atomic non-finalization, and no-mid-round conclusion pass local regression coverage. | `INV-01`, `INV-08`, `INV-10`, `INV-14`–`INV-17` |
| **RESOLVED LOCALLY IN SEQUENCE 5A; OWNER/COMMIT/HOSTED/PILOT GATES OPEN** | Flag-football entry now enforces counterpart presence and equal same-event totals for all four named pairs, including zero-delta attempts. A nonblank reason is required per unpaired event, stored append-only with actor/server time, surfaced again as a projection warning, and bound to the existing reasoned finalization audit. No global bypass exists. | `INV-07`, `INV-15`, `INV-32`, `INV-37` |
