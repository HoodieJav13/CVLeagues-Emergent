...
| **PARTIALLY RESOLVED LOCALLY IN SEQUENCE 5A; STILL BLOCKING before ledger pilot** | The completed-pass control now accepts signed yardage and emits equal `passYards`/`recYards` deltas. The admin surface still lacks a signed standalone rushing-yardage path, so the broader `INV-03` pilot gate remains open. | `INV-03` |
