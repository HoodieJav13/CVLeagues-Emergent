...
// `as` controls both the semantic heading level and its locked display token:
// page-level h1 uses display-xl; the default section h2 uses display-lg.
...
export const SectionHeading = ({ title, subtitle, action, as: Tag = "h2", band = false, tone, className = "" }) => (
  <div className={`flex items-end justify-between gap-4 mb-5 ${className}`}>
    <div>
      {band ? (
        <Tag className={`cvf-band ${tone === "gold" ? "cvf-band--gold" : ""} ${Tag === "h1" ? "text-display-xl" : "text-display-lg"}`}>{title}</Tag>
      ) : (
        <div className="flex items-center gap-2.5 mb-0.5">
          <span className="w-1 h-5 rounded-full bg-primary shrink-0" />
          <Tag className={`font-display uppercase ${Tag === "h1" ? "text-display-xl" : "text-display-lg"} text-foreground`}>{title}</Tag>
        </div>
      )}
