# Hosted authorization matrix evidence — 2026-08-05

- **Overall:** PASS
- **Project:** `orlhqewzprjadyrdrqxw`
- **Surface:** `m30` — Migration 30 (practice-mode boundary, sessions without games)
- **Expected census:** 30 migrations · 28 tables · 33 admin RPCs
- **Observed hosted ledger:** 30 migrations applied, latest `20260729182047` — verified against the local sequence before baseline capture
- **Fixture game shape:** `starts_at` (venue seeded)
- **Run namespace:** `cvf-matrix-2026-08-05-e800712c`
- **Executed:** 2026-08-05T01:20:32.485Z to 2026-08-05T01:21:32.702Z
- **Browser/API checks:** 283 passed, 0 failed
- **Server catalog checks:** 8 passed, 0 failed
- **Combined checks:** 291 passed, 0 failed
...
|---|---:|---:|
| ledger catalog | 8 | 0 |
| MFA authorization | 51 | 0 |
| identity | 3 | 0 |
| ledger authorization | 48 | 0 |
| protected intake | 4 | 0 |
| public reads | 16 | 0 |
| private reads | 18 | 0 |
| migration 29 authorization | 9 | 0 |
...
| MFA authorization | linked password-only administrator cannot execute start_practice_session | PASS | Rejected by the assert_admin() guard. |
| MFA authorization | linked password-only administrator cannot execute append_practice_event | PASS | Rejected by the assert_admin() guard. |
| MFA authorization | linked password-only administrator cannot execute renew_practice_session | PASS | Rejected by the assert_admin() guard. |
| MFA authorization | linked password-only administrator cannot execute resume_practice_session | PASS | Rejected by the assert_admin() guard. |
| MFA authorization | linked password-only administrator cannot execute cancel_practice_session | PASS | Rejected by the assert_admin() guard. |
| MFA authorization | linked password-only administrator cannot execute finalize_practice_session | PASS | Rejected by the assert_admin() guard. |
| MFA authorization | linked password-only administrator cannot execute start_practice_correction | PASS | Rejected by the assert_admin() guard. |
...
| migration 29 authorization | anonymous cannot set participation by RPC | PASS | Denied at the database authorization boundary. |
...
| RPC denial | anonymous cannot execute start_practice_session | PASS | Denied at the database authorization boundary. |
| RPC denial | non-admin cannot execute start_practice_session | PASS | Rejected by the assert_admin() guard. |
| RPC denial | anonymous cannot execute append_practice_event | PASS | Denied at the database authorization boundary. |
| RPC denial | non-admin cannot execute append_practice_event | PASS | Rejected by the assert_admin() guard. |
| RPC denial | anonymous cannot execute renew_practice_session | PASS | Denied at the database authorization boundary. |
| RPC denial | non-admin cannot execute renew_practice_session | PASS | Rejected by the assert_admin() guard. |
| RPC denial | anonymous cannot execute resume_practice_session | PASS | Denied at the database authorization boundary. |
| RPC denial | non-admin cannot execute resume_practice_session | PASS | Rejected by the assert_admin() guard. |
| RPC denial | anonymous cannot execute cancel_practice_session | PASS | Denied at the database authorization boundary. |
| RPC denial | non-admin cannot execute cancel_practice_session | PASS | Rejected by the assert_admin() guard. |
| RPC denial | anonymous cannot execute finalize_practice_session | PASS | Denied at the database authorization boundary. |
| RPC denial | non-admin cannot execute finalize_practice_session | PASS | Rejected by the assert_admin() guard. |
| RPC denial | anonymous cannot execute start_practice_correction | PASS | Denied at the database authorization boundary. |
| RPC denial | non-admin cannot execute start_practice_correction | PASS | Rejected by the assert_admin() guard. |
