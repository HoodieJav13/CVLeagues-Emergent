...
- Twenty-nine migration files are present in filename order. A clean isolated reset applies all twenty-nine, and the independent pgtest run passes 340/340 plus a real two-connection idempotency race. Migration 28 (Sequence 5A overtime/pairing) is published and authorization-accepted; Migration 29 (venues, authoritative `starts_at`, participation) remains local-only. Hosted is aligned through the first twenty-eight migrations.
- **Migration 29 drops three `games` columns (`date`, `time`, `location`) and is therefore not silently reversible once hosted.** It also adds two tables (`venues`, `game_participation`), adds one RPC (`set_game_participation`), and replaces `schedule_playoff_match`'s signature. It remains independently gated so the irreversible column drop is never conflated with Migration 28's accepted checkpoint.
- Migration 28's accepted 256/256 matrix covers 28 hosted migrations, 26 tables, and 25 administrator RPCs. The combined local stack presents 28 tables and 26 RPCs, and **one** RPC signature changes beyond the accepted surface: Migration 29 replaces `schedule_playoff_match`. The matrix must run again after Migration 29 with `--surface m29`; the preserved `m28` mode and its exact hosted-ledger assertion make Migration 28's separate acceptance independently auditable.
- **The CLI cannot publish a subset.** `supabase db push` applies every pending migration and `--include-all` only broadens that set, so isolation comes from what the checked-out migrations directory contains. Migration 28 is now accepted; before advancing `main` to the reconciliation head, confirm that no automatic deployment can fire. The ordered procedure, worktree requirements, and no-deployment stop condition are in [`HOSTED_AUTH_RUNBOOK.md`](HOSTED_AUTH_RUNBOOK.md).
- **The interval between the two publications is a deliberate incompatibility window.** Once `main` advances, its frontend reads `starts_at`/`venue_id` while hosted is still at Migration 28 and has neither. It is safe only because nothing is deployed from it. Advancing `main` requires first confirming that no automatic deployment can fire; if that cannot be ruled out, the sequence stops before `main` moves.
...
- The linked hosted project has all twenty-eight migrations applied. Migration 24's private Event Ledger Lite boundary remains behaviorally accepted, Migrations 25–27 passed migration, structure, row/settings baseline, privilege catalog, and advisor readback, and Migration 28 passed its separate 256/256 authorization matrix. The Season 1 operational baseline was preserved exactly.
...
The project is linked and all twenty-eight hosted migrations are applied with migration history, structural catalog checks, the operational row/settings baseline, and both advisors read back. Behavioral authorization acceptance is current through Migration 28's 256/256 surface. See the accepted [`Migration 28 execution`](evidence/hosted-auth-matrix-2026-07-28-m28.md), the prior [`2026-07-25 execution`](evidence/hosted-auth-matrix-2026-07-24.md), the independent [`2026-07-22 execution`](evidence/hosted-auth-matrix-2026-07-22-m27.md), [`evidence/sequence-4-hosted-push-2026-07-22.md`](evidence/sequence-4-hosted-push-2026-07-22.md), and the immutable earlier checkpoints.
...
| Authenticated executable `SECURITY DEFINER` warnings on helpers and admin RPCs | 27 WARN | Covers the prior two helpers/15 RPCs plus all ten Sequence 4 runtime RPCs. Hosted definitions confirm every new runtime endpoint calls the AAL2-aware admin guard, and the 256/256 matrix verifies lower-role and lower-assurance denial. Positive ledger execution remains a durable-pilot check. |
...
Hosted authorization acceptance is complete and durably evidenced at 256/256 through Migration 28 — see [`HOSTED_AUTH_RUNBOOK.md`](HOSTED_AUTH_RUNBOOK.md) and [`evidence/hosted-auth-matrix-2026-07-28-m28.md`](evidence/hosted-auth-matrix-2026-07-28-m28.md).

Sequence 5A is behaviorally accepted through Migration 28. Migration 29 remains local-only and requires its own backup, deployment-safety check, named hosted-push approval, structural readback, and separately approved `m29` matrix. The remaining signed-rushing-yardage UI path and practice-mode boundary must close before the durable official pilot. Because ledger evidence is intentionally append-only, the hosted positive-row proof must use an explicitly owner-approved durable pilot fixture rather than pretending it can be automatically cleaned up.
...
The hosted ledger currently contains repository Migrations 1–28; local Migration 29 is unpublished and requires its own approved action. Every future migration push, migration-history repair, or other hosted write requires owner approval. Never print or commit access tokens, database passwords, secret keys, or service-role keys.
