...
Frontend was generated via Emergent (React + CRA), polished with a design-system-first UI pass, extended in Claude Code, and given a full visual-token upgrade. The Supabase adapter is env-gated and explicit mock mode remains available for local development. The repository contains twenty-nine migrations and passes 340/340 database assertions plus a real two-connection ledger race locally. The first twenty-eight migrations are hosted, and Migration 28's 26-table / 25-RPC real-session authorization matrix is accepted at 256/256 with zero fixture residue and exact baseline restoration. Sequence 5A overtime is therefore published and authorization-accepted; the player-experience venues/`starts_at`/participation cutover (Migration 29) remains local-only. Migration 29 publication and acceptance, practice and signed-rushing-yardage work, durable populated-ledger proof, and the official flag-football pilot remain. Preview/production acceptance remains open.

## Current Status
- Public site: all pages working; the existing eight-step aggregate score-entry flow is verified in mock mode. The admin live-ledger runtime and UI include overtime continuation/close and paired-stat exception handling; Migration 28 is hosted and authorization-accepted, while Migration 29 remains unhosted and the flag-football pilot is not yet authorized.
...
- Extended-runway backend: all twenty-nine repository migrations apply locally; 340/340 pgtest assertions plus the two-connection append race pass. Hosted is at 28/28 with the current 256/256 authorization matrix and exact Season 1 baseline restoration. Migration 28 (Sequence 5A) is published and authorization-accepted; Migration 29 (venues/`starts_at`/participation) remains local-only, and its publication/acceptance plus the first durable populated-ledger proof remain separately approved gates.
...
- Dedicated hosted backend is linked and structurally aligned at all twenty-eight migrations. The least-privilege service-role catalog, both advisors, Migration 23 RPC-only aggregate boundary, Migration 24 private ledger boundary, and Migration 28's accepted 256/256 real-session/catalog matrix are durably evidenced. Recovery/session-revocation acceptance, the durable ledger pilot, preview/production variables, live application flows, and deployment remain open.

## Current Priority
Reactivation stage R1 closed at `cbfdcc6`, and the four-stage R2 reconciliation is complete. Migration 28 is published and authorization-accepted; the current bounded backend gate is **Migration 29 publication and independent acceptance**, beginning with a fresh off-platform backup and confirmation that advancing `main` cannot trigger an automatic deployment. The player-experience branch is fully integrated; nothing remains unmerged on it. Migration 29's hosted push and fixture-writing matrix each remain separately gated. See [`docs/reconciliation/PLAYER_EXPERIENCE_INTEGRATION_2026-07-26.md`](docs/reconciliation/PLAYER_EXPERIENCE_INTEGRATION_2026-07-26.md), [`supabase/HOSTED_AUTH_RUNBOOK.md`](supabase/HOSTED_AUTH_RUNBOOK.md), and [`docs/scoring/REACTIVATION_DECISION_2026-07-25.md`](docs/scoring/REACTIVATION_DECISION_2026-07-25.md). Attorney-approved New Mexico waiver text remains an independent launch blocker.
...
- Complete Migration 29's separately gated publication and authorization acceptance: fresh off-platform backup, deployment-safety preflight, reviewed `main` fast-forward, filename-exact dry run, named hosted-push approval, structural readback, then a separate fixture-matrix approval. Migration 28's approval does not carry forward.
...
- Backend: Supabase (PostgreSQL + Auth); twenty-nine migrations are verified locally by the isolated harness and the first twenty-eight are applied to the dedicated hosted project. The real administrator, Migration 28's 256/256 current-surface authorization matrix, least-privilege service-role boundary, and hosted advisors are accepted. Migration 29 publication/acceptance, the durable ledger pilot, preview/production configuration, and live-flow acceptance remain open.
...
## Backend Data Model (twenty-nine repository migrations; twenty-eight hosted; authorization accepted through Migration 28)
...
9e. ✅ Player-experience reconciliation (R2) — staged local integration of `claude/sports-league-improvements-esu4eb`, complete across four separately approved stages at 340/340 database assertions plus concurrency, 208/208 frontend tests across 38 suites, 21/21 API tests, and a passing production build. At R2 completion no hosted action had occurred; Migration 28 was subsequently published and authorization-accepted, while Migration 29 remains unpublished.
