...
Frontend was generated via Emergent (React + CRA), polished with a design-system-first UI pass, extended in Claude Code, and given a full visual-token upgrade. The Supabase adapter is env-gated and explicit mock mode remains available for local development. The repository contains thirty migrations and passes 382/382 database assertions plus two real two-connection races locally (the ledger append race and the practice correction-fork race). All thirty migrations are hosted and independently authorization-accepted through Migration 30: published 2026-08-04 under its own token with a passing structural readback ([`supabase/evidence/m30-hosted-push-2026-08-04.md`](supabase/evidence/m30-hosted-push-2026-08-04.md)) and accepted 2026-08-05 at **291/291** with zero fixture residue and exact baseline restoration ([`supabase/evidence/hosted-auth-matrix-2026-08-04-m30.md`](supabase/evidence/hosted-auth-matrix-2026-08-04-m30.md)). Migration 28's independent 26-table / 25-RPC real-session matrix is accepted at 256/256, and Migration 29's 28-table / 26-RPC matrix is independently accepted at 270/270, both with zero fixture residue and exact baseline restoration. Sequence 5A overtime and the player-experience venues/`starts_at`/participation cutover are therefore published and authorization-accepted. The signed standalone rushing-yardage path added no migration. Practice mode is reviewed, merged, hosted, and authorization-accepted; the durable populated-ledger proof and the official flag-football pilot remain. Preview/production acceptance remains open.
...
- Extended-runway backend: all thirty repository migrations apply locally; 382/382 pgtest assertions plus the two-connection append race and the practice fork race pass. Hosted is at 30/30 with authorization acceptance through Migration 30's 291/291 and exact Season 1 baseline restoration. Migrations 28, 29, and 30 are each published and independently authorization-accepted at their own boundaries; the first durable populated-ledger proof remains a separately approved gate.
...
- Dedicated hosted backend is linked and structurally aligned at all thirty repository migrations, authorization-accepted through Migration 30 (291/291, 2026-08-05). The least-privilege service-role catalog, both advisors, Migration 23 RPC-only aggregate boundary, Migration 24 private ledger boundary, Migration 28's accepted 256/256 matrix, and Migration 29's accepted 270/270 real-session/catalog matrix are durably evidenced. Recovery/session-revocation acceptance is complete with a break-glass administrator in place (2026-08-16). The durable ledger pilot, preview/production variables, live application flows, and deployment remain open.

## Current Priority
Reactivation R1, the four-stage R2 reconciliation, Migrations 28 and 29 with independent authorization acceptance, Pass 4 visual elevation in full, Pass 5 motion (Addendum 10), and the signed standalone rushing-yardage path are all complete. `main` and hosted are both at thirty migrations, and **the Migration 30 sequence is complete**: published 2026-08-04 under the owner's `30-30` token after runbook steps 14–17 with every structural readback item passing ([`m30-hosted-push-2026-08-04.md`](supabase/evidence/m30-hosted-push-2026-08-04.md)), then authorization-accepted 2026-08-05 at **291/291** — 283 browser/API plus 8 catalog checks, zero fixture residue, exact baseline restoration ([`hosted-auth-matrix-2026-08-04-m30.md`](supabase/evidence/hosted-auth-matrix-2026-08-04-m30.md)). The 21-check delta over the accepted `m29` 270/270 is exactly the seven practice RPCs across anonymous, authenticated non-admin, and AAL1-admin sessions; every probe reached a real authorization boundary rather than a function lookup. `DEFAULT_SURFACE` was promoted to `m30` in the same change that committed that evidence.
...
- **Leaked-password protection: confirmed Pro-gated (2026-08-06)** — the toggle exists but save fails on the Free plan. Folded into the decided Pro upgrade at deployment: flip it immediately at upgrade time, because it only checks passwords set after enabling. Both current administrator passwords are generated 20+ char strings, which is the interim mitigation.
...
- **`/api/calendar` abuse protection: DECIDED AND IMPLEMENTED (2026-08-06)** — per-IP fixed-window rate limiting (30 requests / 10 minutes) inside the serverless function via Vercel KV / Upstash REST. Missing store configuration on the deployed platform fails closed (503); a transient store failure serves unthrottled and logs. Remaining owner action at deployment: create the free KV store and set `KV_REST_API_URL` / `KV_REST_API_TOKEN`.
...
- **Launch remains blocked on attorney-approved waiver text regardless of all other gates closing.** The ready-to-forward engagement package — league description, exact capture fields, seven NM-specific questions — is at [`docs/legal/WAIVER_ATTORNEY_PACKAGE_2026-08-06.md`](docs/legal/WAIVER_ATTORNEY_PACKAGE_2026-08-06.md); the owner's action is to send it. After approval, insert the final text as a new immutable `waiver_versions` row; never substitute draft or fallback legal text.
- **Recovery/session-revocation acceptance: COMPLETE (Phases 1–3 2026-08-06, Phases 4–5 2026-08-16)** — revocation and recovery proven live (forcing the AAL2 reset fix `5c24d89`), break-glass administrator created, TOTP-enrolled, and stored on paper offline; final state 2 admin links and 2 verified factors with the primary unaffected. Evidence: [`supabase/evidence/recovery-acceptance-2026-08-06.md`](supabase/evidence/recovery-acceptance-2026-08-06.md). The Auth Site URL (`http://localhost:3000`) must become the production domain at the Phase 10 configuration session.
- **Upgrade the Supabase project to the Pro tier at deployment** (decided 2026-08-05): removes the free-plan idle pause that would take the public site down after quiet weeks — observed firsthand on 2026-08-04 — and adds daily automatic backups covering waiver legal records. Until then the free plan is fine because nothing public depends on hosted.
- **Check the Vercel dashboard for stale manual deployments at the Phase 10 configuration session** (decided 2026-08-05): delete any live deployment of an outdated build before entering production env values.
...
- **Durable populated-ledger pilot: specified and deliberately deferred** (2026-08-04) until real Season 1 teams and rosters exist hosted — see [`docs/scoring/DURABLE_LEDGER_PILOT_2026-08-04.md`](docs/scoring/DURABLE_LEDGER_PILOT_2026-08-04.md). A practice rehearsal closes the gate with rows that touch no game, but it PINS the teams, profiles, and roster rows it uses via `on delete restrict`, and hosted is currently an empty scaffold (1 team, 0 games), so running it today would weld fixture teams permanently into production. Once real teams exist the pilot costs nothing and the evidence is better. No approval is needed to wait; the run itself still needs one. The entry order that satisfies its trigger condition is written down in [`docs/operations/SEASON1_DATA_ENTRY_2026-08-05.md`](docs/operations/SEASON1_DATA_ENTRY_2026-08-05.md).
- Field-test the admin-only flag-football pilot before approving a second sport or live use.
...
- Backend: Supabase (PostgreSQL + Auth); thirty migrations are verified locally by the isolated harness and all thirty are applied to and authorization-accepted on the dedicated hosted project. The real administrator, Migration 28's independent 256/256 matrix, Migration 29's current-surface 270/270 matrix, least-privilege service-role boundary, and hosted advisors are accepted. The durable ledger pilot, preview/production configuration, and live-flow acceptance remain open.
...
## Backend Data Model (thirty repository migrations, all hosted and authorization-accepted through Migration 30)
...
External critical-path dependency (unchanged): NM attorney waiver review — the engagement package is prepared and awaiting send. Domain: availability checked 2026-08-06, all six candidates open (`cvfsports.com`, `cvfleagues.com`, `playcvf.com`, `cvfabq.com`, `cvfsportsnm.com`, `cvfsports.org`); owner registers the pick. Other lead-time item: confirm friend's native-app stack.
