...
- Extended-runway backend: all twenty-eight repository migrations apply locally; 318/318 pgtest assertions plus the two-connection append race pass. Hosted remains at 27/27 with the current 256/256 authorization matrix and exact Season 1 baseline restoration. Sequence 5A is committed locally; Migration 28 publication/acceptance and the first durable populated-ledger proof remain separately approved gates.
...
## Current Priority
The owner explicitly lifted the prior freeze for one bounded reactivation stage: reconcile the accepted Sequence 4 baseline at `fa737b2` with the committed, locally verified Sequence 5A work and install the repository Protocol v1.2 contract. Stop after the reviewed local merge commit; do not push, publish Migration 28, begin the durable pilot, or start another roadmap stage without a new owner-approved envelope. See [`docs/scoring/REACTIVATION_DECISION_2026-07-25.md`](docs/scoring/REACTIVATION_DECISION_2026-07-25.md). Attorney-approved New Mexico waiver text remains an independent launch blocker.

## Owner Action Queue

- After the bounded reactivation merge closes, select exactly one next stage and approve its autonomy envelope. Migration 28 publication/acceptance, signed rushing-yard plus practice-mode completion, the durable pilot, Sequence 6, and later UI work remain separate choices.
...
- Backend: Supabase (PostgreSQL + Auth); twenty-eight migrations are verified locally by the isolated harness and the first twenty-seven are applied to the dedicated hosted project. The real administrator, Sequence 4's 256/256 current-surface authorization matrix, least-privilege service-role boundary, and hosted advisors are accepted. Migration 28 publication/acceptance, the durable ledger pilot, preview/production configuration, and live-flow acceptance remain open.
