...
Frontend was generated via Emergent (React + CRA), polished with a design-system-first UI pass, extended in Claude Code, and given a full visual-token upgrade. The Supabase adapter is env-gated and explicit mock mode remains available for local development. The repository contains twenty-nine migrations and passes 340/340 database assertions plus a real two-connection ledger race locally. All twenty-nine migrations are hosted: Migration 28's independent 26-table / 25-RPC real-session matrix is accepted at 256/256, and Migration 29's current 28-table / 26-RPC matrix is independently accepted at 270/270, both with zero fixture residue and exact baseline restoration. Sequence 5A overtime and the player-experience venues/`starts_at`/participation cutover are therefore published and authorization-accepted. The signed standalone rushing-yardage path is complete and added no migration, which is what keeps the count at twenty-nine. Practice mode is the selected next stage, with an unreviewed implementation on a branch and nothing hosted; durable populated-ledger proof and the official flag-football pilot remain. Preview/production acceptance remains open.
...
- Extended-runway backend: all twenty-nine repository migrations apply locally; 340/340 pgtest assertions plus the two-connection append race pass. Hosted is at 29/29 with the current 270/270 authorization matrix and exact Season 1 baseline restoration. Migration 28 (Sequence 5A) and Migration 29 (venues/`starts_at`/participation) are published and independently authorization-accepted; the first durable populated-ledger proof remains a separately approved gate.
...
- Dedicated hosted backend is linked and structurally aligned at all twenty-nine migrations. The least-privilege service-role catalog, both advisors, Migration 23 RPC-only aggregate boundary, Migration 24 private ledger boundary, Migration 28's accepted 256/256 matrix, and Migration 29's accepted 270/270 real-session/catalog matrix are durably evidenced. Recovery/session-revocation acceptance, the durable ledger pilot, preview/production variables, live application flows, and deployment remain open.

## Current Priority
Reactivation R1, the four-stage R2 reconciliation, Migrations 28 and 29 with independent authorization acceptance, Pass 4 visual elevation in full, Pass 5 motion (Addendum 10), and the signed standalone rushing-yardage path are all complete. `main` is at twenty-nine migrations, hosted is at the same twenty-nine, and there is no unhosted migration debt.

**The next bounded stage is practice mode, Option B — sessions without games**, selected from [`docs/scoring/PRACTICE_MODE_DESIGN_2026-07-29.md`](docs/scoring/PRACTICE_MODE_DESIGN_2026-07-29.md), which recommended it. Practice becomes a `session_kind` on the existing private ledger tables with `game_id` nullable only for practice, per-session event sequencing, and a finalization that computes and returns a projection while writing nothing to `games`, `player_stats`, or `game_edit_history`. Exclusion is structural rather than filtered: practice data lives only in the four admin-only private tables and never references a game. The cost is that it is invasive within the subsystem — composite `(id, game_id)` foreign keys and per-game sequencing need practice-aware parallels, and all ten runtime RPCs need practice guards. It is therefore a proper Migration 30 sequence with its own negative matrix, not a rider on existing work.

**An unreviewed implementation already exists on branch `practice-mode-migration30`.** Two facts are recorded here without asserting a causal order between them, because the repository cannot establish one: Migration 30 and an admin practice rehearsal UI were committed on 2026-07-29 (`a9de3ab` and `10ad297` — 1,157 migration lines, 371/371 local assertions claimed), and the written selection of Option B was committed on 2026-08-01 (`781aafd`, on `origin/main`'s `3ce99de`, superseded by this entry). Whether a conversational approval preceded the 7/29 build is not recoverable from the repository and no claim is made either way; the owner does not recall. The branch is therefore treated as an unreviewed proposal implementation rather than as work completed under an authorized stage, and its own copy of the design doc still reads "no implementing code," which is false on that branch.

**The selection carries nothing downstream.** Independent review of the branch diff, local pgtest acceptance on a clean replay, an `m30` entry in the two-surface harness contract ([`tests/hosted-auth/surface_contract.test.mjs`](tests/hosted-auth/surface_contract.test.mjs) and [`tests/hosted-auth/run_matrix.sh`](tests/hosted-auth/run_matrix.sh), which currently accept only `m28` and `m29` and deliberately fail loudly on `m30`), and then the full ordered procedure in [`supabase/HOSTED_AUTH_RUNBOOK.md`](supabase/HOSTED_AUTH_RUNBOOK.md) — fresh off-platform export, deployment-safety confirmation, filename-exact dry run, a new literal approval token, structural readback, and a separately approved fixture-writing matrix run — are each separate gates.
...
- **Practice mode Option B is the selected next bounded stage; its implementation is not authorized.** An unreviewed implementation exists on `practice-mode-migration30` (see Current Priority). Decide whether to ratify that branch after independent review or rebuild the stage under ordinary discipline. Local acceptance, the `m30` harness surface entry, and Migration 30's hosted push are separate gates that the selection does not carry.
...
- Backend: Supabase (PostgreSQL + Auth); twenty-nine migrations are verified locally by the isolated harness and all twenty-nine are applied to the dedicated hosted project. The real administrator, Migration 28's independent 256/256 matrix, Migration 29's current-surface 270/270 matrix, least-privilege service-role boundary, and hosted advisors are accepted. The durable ledger pilot, preview/production configuration, and live-flow acceptance remain open.
...
## Backend Data Model (twenty-nine repository migrations; twenty-nine hosted; authorization accepted through Migration 29)
