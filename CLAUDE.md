...
Frontend was generated via Emergent (React + CRA), polished with a design-system-first UI pass, extended in Claude Code, and given a full visual-token upgrade. The Supabase adapter is env-gated and explicit mock mode remains available for local development. The repository contains thirty migrations and passes 382/382 database assertions plus two real two-connection races locally (the ledger append race and the practice correction-fork race). Twenty-nine migrations are hosted — Migration 30, the practice-mode boundary, is local-only pending its own approved push: Migration 28's independent 26-table / 25-RPC real-session matrix is accepted at 256/256, and Migration 29's current 28-table / 26-RPC matrix is independently accepted at 270/270, both with zero fixture residue and exact baseline restoration. Sequence 5A overtime and the player-experience venues/`starts_at`/participation cutover are therefore published and authorization-accepted. The signed standalone rushing-yardage path added no migration. Practice mode is reviewed and merged on `main` with its `m30` matrix surface prepared; durable populated-ledger proof and the official flag-football pilot remain. Preview/production acceptance remains open.
...
- Extended-runway backend: all thirty repository migrations apply locally; 382/382 pgtest assertions plus the two-connection append race and the practice fork race pass. Hosted is at 29/30 with the current 270/270 authorization matrix and exact Season 1 baseline restoration. Migration 28 (Sequence 5A) and Migration 29 (venues/`starts_at`/participation) are published and independently authorization-accepted; Migration 30 (practice mode) awaits its own approved push; the first durable populated-ledger proof remains a separately approved gate.
...
- Dedicated hosted backend is linked and structurally aligned at twenty-nine of the thirty repository migrations (Migration 30 awaits its own approved push). The least-privilege service-role catalog, both advisors, Migration 23 RPC-only aggregate boundary, Migration 24 private ledger boundary, Migration 28's accepted 256/256 matrix, and Migration 29's accepted 270/270 real-session/catalog matrix are durably evidenced. Recovery/session-revocation acceptance, the durable ledger pilot, preview/production variables, live application flows, and deployment remain open.

## Current Priority
Reactivation R1, the four-stage R2 reconciliation, Migrations 28 and 29 with independent authorization acceptance, Pass 4 visual elevation in full, Pass 5 motion (Addendum 10), and the signed standalone rushing-yardage path are all complete. `main` is at thirty migrations; hosted is at twenty-nine. Migration 30 is the one unhosted migration, deliberately: its hosted push is a separately approved gate.
...
**Local acceptance stands at 382/382 pgtest assertions plus two concurrency races, 81/81 harness contract tests, 217/217 frontend tests across 39 suites, and a clean production build.** The `m30` matrix surface exists in the two-surface harness contract ([`tests/hosted-auth/surface_contract.test.mjs`](tests/hosted-auth/surface_contract.test.mjs), [`tests/hosted-auth/run_matrix.sh`](tests/hosted-auth/run_matrix.sh)): seven RPCs, zero new tables, with `DEFAULT_SURFACE` deliberately still `m29` because promoting it is a consequence of the hosted push, never a precondition. **What remains for Migration 30 is entirely gated on the owner:** the full ordered procedure in [`supabase/HOSTED_AUTH_RUNBOOK.md`](supabase/HOSTED_AUTH_RUNBOOK.md) — fresh off-platform export, deployment-safety confirmation, filename-exact dry run, a new literal approval token, structural readback — and then a separately approved fixture-writing `m30` matrix run.
...
- **Approve Migration 30's hosted push when ready.** Practice mode is reviewed, merged, and locally accepted (see Current Priority); the runbook procedure and a new literal approval token remain yours, followed by the separately approved fixture-writing `m30` matrix run. Until then hosted deliberately stays at twenty-nine.
...
- Backend: Supabase (PostgreSQL + Auth); thirty migrations are verified locally by the isolated harness and twenty-nine are applied to the dedicated hosted project (Migration 30 pending its approved push). The real administrator, Migration 28's independent 256/256 matrix, Migration 29's current-surface 270/270 matrix, least-privilege service-role boundary, and hosted advisors are accepted. The durable ledger pilot, preview/production configuration, and live-flow acceptance remain open.
...
## Backend Data Model (thirty repository migrations; twenty-nine hosted; authorization accepted through Migration 29; Migration 30 practice-mode boundary local-only)
