...
Frontend was generated via Emergent (React + CRA), polished with a design-system-first UI pass, extended in Claude Code, and given a full visual-token upgrade. The Supabase adapter is env-gated and explicit mock mode remains available for local development. The repository contains twenty-nine migrations and passes 338/338 database assertions plus a real two-connection ledger race locally. The first twenty-seven migrations are hosted, and Sequence 4's 26-table / 25-RPC real-session authorization matrix is accepted at 256/256 with zero fixture residue and exact baseline restoration. Sequence 5A overtime (Migration 28) and the player-experience venues/`starts_at`/participation cutover (Migration 29) are committed locally; publication and acceptance of both, practice and signed-rushing-yardage work, durable populated-ledger proof, and the official flag-football pilot remain. Preview/production acceptance remains open.

## Current Status
- Public site: all pages working; the existing eight-step aggregate score-entry flow is verified in mock mode. The admin live-ledger runtime and UI now include committed, locally verified overtime continuation/close and paired-stat exception handling, while Migrations 28 and 29 remain unhosted and the flag-football pilot is not yet authorized.
- Player-experience reconciliation (R2): derived-stat engine, participation-aware selectors, standings form/streak/shared tie ranks, and the full public profile are integrated (R2-A). Migration 29's venues, authoritative `starts_at`, and participation plus the complete frontend cutover are integrated (R2-B). Admin venue management, matrix expansion, and calendar work remain unintegrated on the feature branch.
...
- Extended-runway backend: all twenty-nine repository migrations apply locally; 338/338 pgtest assertions plus the two-connection append race pass. Hosted remains at 27/27 with the current 256/256 authorization matrix and exact Season 1 baseline restoration. Migrations 28 (Sequence 5A) and 29 (venues/`starts_at`/participation) are committed locally and unhosted; their publication/acceptance and the first durable populated-ledger proof remain separately approved gates.
...
## Current Priority
Reactivation stage R1 closed at `cbfdcc6`. The active work is **R2: staged local reconciliation of the player-experience branch** (`claude/sports-league-improvements-esu4eb`) with `main`, one owner-approved stage at a time. R2-A (derived stats, participation-aware selectors, standings form/streak/tie ranks, full public profile) and R2-B (Migration 29 venues/`starts_at`/participation plus its frontend cutover) are complete and pushed. R2-C (admin venue management, matrix expansion, franchise history) and R2-D (calendar export and feed) each require separate approval. Every hosted action — including publishing Migration 28 or 29 — remains separately gated. See [`docs/reconciliation/PLAYER_EXPERIENCE_INTEGRATION_2026-07-26.md`](docs/reconciliation/PLAYER_EXPERIENCE_INTEGRATION_2026-07-26.md) and [`docs/scoring/REACTIVATION_DECISION_2026-07-25.md`](docs/scoring/REACTIVATION_DECISION_2026-07-25.md). Attorney-approved New Mexico waiver text remains an independent launch blocker.
...
- Approve or decline R2-C and R2-D individually. Migration 28 and 29 publication/acceptance, signed rushing-yard plus practice-mode completion, the durable pilot, Sequence 6, and later UI work remain separate choices.
- **Decide the `/api/calendar` abuse-protection question before R2-D.** The feed is an unauthenticated public endpoint reading publicly readable rows with the anonymous key. It has a 30-minute cache but no rate limiting, and response caching is not abuse protection. `AGENTS.md` requires abuse protection on anonymous surfaces before public launch, so this needs a decision at R2-D rather than at deployment.
...
- Backend: Supabase (PostgreSQL + Auth); twenty-nine migrations are verified locally by the isolated harness and the first twenty-seven are applied to the dedicated hosted project. The real administrator, Sequence 4's 256/256 current-surface authorization matrix, least-privilege service-role boundary, and hosted advisors are accepted. Migration 28 and 29 publication/acceptance, the durable ledger pilot, preview/production configuration, and live-flow acceptance remain open.
...
### Event Ledger Lite correction contract (Sequence 5A committed locally; durable pilot pending)
...
- The committed, local-only Migration 28 adds explicit append-only overtime period closes, tied/open continuation without publication, paired-stat counterpart enforcement, and per-event reasoned exception evidence that resurfaces at finalization. It passes the clean 28-migration replay, 318/318 assertions plus concurrency, 133/133 frontend tests, production build, and responsive browser verification; hosted publication and acceptance remain pending.
...
## Backend Data Model (twenty-nine repository migrations; twenty-seven hosted; authorization accepted through Migration 27)
...
9c. ▶ Scorekeeping integrity program — Stages 0–1 are committed; Stage 2 aggregate hardening and Stage 3 Event Ledger Lite schema are hosted-accepted. Stage 4A–4C runtime/projection/correction is hosted-authorization accepted after 294/294 plus a two-connection race, 128/128 frontend tests, 10/10 matrix contract tests, hosted structural/catalog/advisor readback, and the 256/256 current-surface matrix with exact restoration. Stage 5A overtime + `INV-07` is committed locally at 318/318 plus concurrency and 133/133 frontend tests; Migration 28 publication/acceptance, signed standalone rushing-yard entry, practice mode, durable pilot, and Stage 6 field test/rollout remain.
9d. ◐ Pass 4 visual elevation — Batches 0–2.5 complete; Team/Profile, Playoffs, and forms resume after the scorekeeping pilot decision. Carried into the Team/Profile batch: the Standings `LAST 5` form chips are logged VISUALLY INSUFFICIENT against Anchor B (16px chips, 10px text, 60%-opacity borders under-register, worst on the gold leader row); proposed direction is 20px chips, 11px text, full-opacity borders, and a contrast pass against `--leader-bg`.
9e. ▶ Player-experience reconciliation (R2) — staged local integration of `claude/sports-league-improvements-esu4eb`. R2-A and R2-B complete at 338/338 database assertions plus concurrency, 160/160 frontend tests, 9/9 API tests, and a passing production build. R2-C (venue admin, matrix expansion, franchise history) and R2-D (calendar) each need separate approval. No hosted action taken.
